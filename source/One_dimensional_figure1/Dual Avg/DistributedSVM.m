function [fvals meanFits] = DistributedSVM(B, A, X, T, stepsize, R)
...
for t = 1:T
  fvals(t) = CalculateFitness_real(y');
 %%
  for i=1:n
    fits(i) = CalculateFitness_real(X(:,i)');
  end
 ...
 for i=1:n
    for j=1:d
        G(j,i) = GradientOfCalculateFitness_real(X(:,i)');
    end
 end
 %%%inprods = dot(B', X)';  
 %   G(:) = 0;
...
fvals(T + 1) = CalculateFitness_real(y');
%%
for i=1:n
    fits(i) = CalculateFitness_real(X(:,i)');
end
