...
if (scaling_by_diameter)
  ...
  T_grid = ceil(T * (1 - norm(P_expander - ones(n) / n)) / ...
		(1 - norm(P_grid - ones(n) / n)));
  T_cycle = ceil(T * (1 - norm(P_expander - ones(n) / n)) / ...
		 (1 - norm(P_cycle - ones(n) / n)) / 2);
  T_expander = T;
end
...
for test_iter = 1:numtests
  ...
  obj_vals = DistributedSVM(B, A_cycle, zeros(dim, 1), T_cycle, eta_cycle, R,optimal);
  res = obj_vals - 1;
  cycle_opt_gaps(test_iter, :) = res;
  figure;
  plot(res);
end
figure;
plot(mean(cycle_opt_gaps));

figure;
errorbar(mean(cycle_opt_gaps),std(cycle_opt_gaps));
